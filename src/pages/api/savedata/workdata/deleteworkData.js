...
export default async function API_DeleteWorkData(req, res) {
  try {
    ...
    //取得使用者id
    const { userid, UserAccessToken, GuildId, OldDataName } = req.body;
    ...
    //請求成員伺服器
    const DeleteWorkData = await WorkSomething.findOneAndDelete(
      { guild: GuildId, name: OldDataName },
    )
    //返回資料
    if(!DeleteWorkData) return res.status(500).json({ message: 'An unknown error occurred while processing the data'})
    return res.status(200).json(DeleteWorkData);
  } catch (error) {
    console.log(error);
